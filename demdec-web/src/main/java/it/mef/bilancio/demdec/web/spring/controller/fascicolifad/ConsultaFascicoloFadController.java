...
//		ConsultaFascicoloFadForm form = context.getForm();
	...
	*/
	public String onApriFascicoloFad(Context context) throws Throwable{
		...
		//
		List<FascicoloSipatrTO> fascicoliPat = fascicoloDemBil!=null?fascicoloDemBil.getFascicoliSIPATR():null;
//		fascicoliPat = gestioneFadManager.getFascicoliPatrimonio(to.getCodiGuidFascicolo()).getListoutputTO();
...
//		ConsultaFascicoloFadForm form = context.getForm();
		...
		if (!StringUtil.isEmpty(to.getDocumentiRedList())){
			...
			AnagUfficiTO uffCreatore= getUtenteInSessione().getUtenteProfiloInUso().getAnagUffici();
			if (uffCreatore.getId().equals(WebConstants.UFFICIO_II)) {
...
//		ConsultaFascicoloFadForm form = context.getForm();
	...
	protected void popolaPreview(FascicoliTO to, FascicoloAttoDecretoTO fascicoloDemBil){
		for (DocumentiTO doc : to.getDocumentiList()){
			for (DocumentTO docDemBil : fascicoloDemBil.getDocumenti()){
				if (doc.getCodiGuidDocumentoOrig().equalsIgnoreCase(docDemBil.getIdDocumento())){
					doc.setPreView(docDemBil.getPreView());
					break;
				}
			}
		}
		for (SottoFascicoliRedTO sfTo : to.getSottoFascicoliRedList()){
			for (DocumentiRedTO doc:sfTo.getDocumentiRedList()){
				for (DocumentTO docDemBil : fascicoloDemBil.getAllegatiDecretoIGB()){
					if (doc.getId().getCodiGuidDocumento().equalsIgnoreCase(docDemBil.getIdDocumento())){
						doc.setPreView(docDemBil.getPreView());
						break;
					}
				}
