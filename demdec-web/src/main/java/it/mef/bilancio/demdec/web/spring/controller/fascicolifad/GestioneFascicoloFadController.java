...
//		}
		...
		AnagUfficiTO uff = getUtenteInSessione().getUtenteProfiloInUso().getAnagUffici();
		if (uff!=null && uff.getId().equals(WebConstants.UFFICIO_II)) {
...
//		ConsultaFascicoloFadForm form = context.getForm();
		...
		if (!StringUtil.isEmpty(to.getDocumentiRedList())){
			...
			if (uffCreatore!=null && uffCreatore.getId().equals(WebConstants.UFFICIO_II)) {
				context.setDataModel(WebConstants.CHK_MODIFICA_VISIBILITA, true);
			}else{
				context.setDataModel(WebConstants.CHK_MODIFICA_VISIBILITA, false);
			}
	...
	public String onModificaMetadatiDocRed(Context context) throws Throwable{
		GestioneFascicoloFadForm form = context.getForm();
		//prelevo il documento da modificare;
		DocumentiRedTO to = (DocumentiRedTO)context.getModel(DOCUMENTO_RED);
		if (to.getFlagVisibilitaCdc01().intValue()==Integer.parseInt(form.getModaleUpdRedVisibilitaCdc())
			&& to.getDescDocumento().getId().equalsIgnoreCase(form.getModaleUpdRedDescDoc())){
		...
		DescDocumentoTO descTo = getGestioneFadManager().findVisibilitaDocumentoByCodiDescDocumento(form.getModaleUpdRedDescDoc());
		to.setDescDocumento(descTo);
