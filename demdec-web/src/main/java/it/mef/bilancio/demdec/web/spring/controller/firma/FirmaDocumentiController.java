...
public class FirmaDocumentiController extends AbstractDemFormController {
	...
	public String firma(Context context) throws Throwable{
		...
		if(form.isValid()){
			...
			for (int i = 0; i < form.getSelezioni().length; i++) {
				...
				context.valorizeAudit(selectedDocument, NomeFunzioneConstants.FIRMA_DOCUMENTI);
				try {
					IterFirmaDecretoTO ifd = documentiManager.firma(selectedDocument, form.getPin(), getUtenteInSessione(), context.getRequest(), context.getResponse(), form.getSelezioni().length);
					String email = "";
					...
					if(ifd != null){
						...
						message = mailManager.invioPeoUltimaFirma(NomeFunzioneConstants.ULTIMA_FIRMA, getUtenteInSessione(), email, selectedDocument);
					}
					...
					if(message != null){
						...
						context.addInfo("info.doc.signed.and.msg.sent", selectedDocument.getDescNomeDocumento(), mailManager.recipientsToString( message.getAllRecipients()));
					}else{
						context.addInfo("info.doc.signed", selectedDocument.getDescNomeDocumento());
	...
	public String findMySignedDocuments(Context context) throws Throwable{
		...
		List<DocumentiViewTO> listMySignedDocuments = documentiManager.listSignedADByLoggedUser(getUtenteInSessione(), Constants.FIRMA);
...
//		List<DocumentiViewTO> listADSigned = documentiManager.listSignedADByLoggedUser(getUtenteInSessione(), Constants.FIRMA);
