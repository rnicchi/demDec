...
<!-- PageName = mieiDocumentiVerificati.jsp -->
...
<fieldset>
	...
	<demdec:displaytag-table name="pageScope.listMySignedDocuments" id="listMySignedDocuments" class="lux width100">
		<display:caption><span class="nascosto"><spring:message code="label.title.list.AD.verificati"/></span></display:caption>
		...
		<demdec:displaytag-column property="rowUpdatedDttm" format="{0,date,dd/MM/yyyy}" titleKey="label.data.verifica" title="label.data.verifica" class="center"/>
		...
		<demdec:displaytag-column title="" titleKey="label.fad" class="center">
			<demdec:linkButton styleClass="center" action="/private/consultaFascicoloFAD.do" parameter="apriFascicoloFad&signed='V'&funzione=${action}&idFad" parameterValue="${listMySignedDocuments.fascicoli.id}" imageKey="ico.fad.show"  labelKey="alt.apri.fascicoloFad"/>
		</demdec:displaytag-column>
	...
	<c:if test="${paginator ne null}">
     ...
     <div class="center color_grey">
         <demdc:paginator controllerCode="<%=Paginator.PARAMETER_PAGINATOR%>"
