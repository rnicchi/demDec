...
public class FascicoloDaoImpl extends FascicoloParentDaoImpl implements FascicoloDao {
...
//		SimpleDateFormat sdf=new SimpleDateFormat("dd/MM/yyyy");
		...
		q+=" and stat_stato IN (0,3) and nume_patrimonio is null ";
		//q+=" ORDER BY fk_anno_ese, fk_nume_tipo_decreto, nume_num_decreto";
		q+=" ORDER BY to_number(annoEse) desc, to_number(numeNumDecreto) asc, descSigla asc ";
...
*/
	...
	public List<FascicoliBO> findFascicoliGestioneFad(FascicoliFadSearhTO to)
		...
		if(to.getStato() == null || "".equals(to.getStato())){
...
*/
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<FascicoliBO> findFascicoliFadPecPartecipazione(FascicoliFadSearhTO to) throws Throwable {
		
		Criteria cri = getSession().createCriteria(FascicoliBO.class);
		cri.add(Restrictions.eq("eserciziDem.id", to.getAnnoDecreto()));
		cri.add(Restrictions.isNotNull("dataRegistrazioneCdc"));
		...
		addCreateEq(cri,"numePatrimonio", to.getNumePat());
		addCreateEq(cri, "statStato", to.getStato());
		addCreate(cri,"dataRegistrazioneCdc", to.getDataRegistrazioneCdcDa(), "ge");
		addCreate(cri,"dataRegistrazioneCdc", to.getDataRegistrazioneCdcA(), "le");
		addCreateEq(cri,"flagVisibilitaCdc01", to.getFlagVisibilitaCdc01());
	...
	{	
		return (RiepilogoDatiAnagBO)findUniqueByNamedParam("from RiepilogoDatiAnagBO a join fetch a.anagTipoRiepilogo where fk_nume_id_fascicolo =:idFascicolo","idFascicolo",id);	
	}

	public RiepilogoDatiAnagBO saveAggiornamentoFascicolo(RiepilogoDatiAnagBO bo) throws Throwable
	{	
		return (RiepilogoDatiAnagBO)saveOrUpdate(bo);	
	}
	
	
	public RiepilogoDatiAnagBO saveAggiornamentoFascicoloTO(RiepilogoDatiAnagTO to, AnagTipoRiepilogoBO tipoBo) throws Throwable
	{	
		
		RiepilogoDatiAnagBO bo = new RiepilogoDatiAnagBO();
		...
		bo.setId(to.getId());
		bo.setDescRiepilogo(to.getDescRiepilogo());
		bo.setFlagIndebitamentoSn(to.getFlagIndebitamentoSn());
		
		bo.setBinaIntReport(to.getBinaIntReport());
		
		bo.setAnagTipoRiepilogo(tipoBo);
		
		FascicoliBO fascicolo = loadById(to.getFascicoli().getId());
		bo.setFascicoli(fascicolo);
		
		bo.setNumeVersion(to.getNumeVersion());
		
		
		//commentata la <!--<generator class="sequence">--> in 	RiepilogoDatiAnagBO.hbm.xml
		//altrimenti non funziona il saveOrUpdate(bo) in inserimento dati (in update andava comunque)
		return (RiepilogoDatiAnagBO)saveOrUpdate(bo);	
	}
