	...
	<c:if test="${not empty listDecretiMonitoraggio}">
		...
		<demdec:displaytag-table class="lux tableDati width100" name="pageScope.listDecretiMonitoraggio" id="listDecreti" >
			...
			<demdec:displaytag-column titleKey="label.monitoraggioFirma.table.stato" class="center">
				...
				<a href="javascript:this.document.getElementById('apri${listDecreti.id}').click();">
					<c:if test="${listDecreti.statStato eq 2}">
						<img class="padd_sx_logo" src="<c:url value="/public/img/ico_sign_yell.png"/>" alt="In firma" title="In firma"/>
					</c:if>
					<c:if test="${listDecreti.statStato eq 3}">
						<img class="padd_sx_logo" src="<c:url value="/public/img/ico_sign_red.png"/>" alt="Sospeso per integrazione" title="Sospeso"/>
					</c:if>
			...
			<demdec:displaytag-column property="codiDecreto" titleKey="label.monitoraggioFirma.table.decreto"/>
