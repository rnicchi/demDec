		...
		<demdec:displaytag-table skipDiv="true"  name="pageScope.listDocumentiFascicolo" id="listDocumentiFascicolo" >
			...
			<demdec:displaytag-column  titleKey="label.raccolteProvvisorie.preView"  class="center">
 				...
 				<c:if test="${not empty listDocumentiFascicolo.preView}">
					<a href="javascript: caricaModaleAnteprima('${listDocumentiFascicolo_rowNum -1}','<%=SessionAttributes.LIST_DOCUMENTI_RED%>'); " >
...
<%-- 				<demdec:linkButton styleClass="padd_sx_logo" action="${action}" imageKey="ico.download.pdf" parameter="scaricaPdfRed" parameterValue="${listDocumentiFascicolo_rowNum -1}"/> --%>
				<c:if test="${listDocumentiFascicolo.codiGuidDocumentoTrasf ne null}">
					<demdec:linkButton  action="${action}" imageKey="ico.download.pdf" parameter="scaricaPdfRed"  title="${listDocumentiFascicolo.descFilenameTrasf}"  parameterValue="${listDocumentiFascicolo_rowNum -1}"/>
				</c:if>
...
<%-- 			<demdec:displaytag-column property="" titleKey="label.raccolteProvvisorie.modificatoDa"  /> --%>
			<demdec:displaytag-column titleKey="label.raccolteProvvisorie.visualizzaMetadati" class="text-center">
				...
				<a href="javascript: visualizzaModaleMetadatiDocRed('contModalVis_0', '${listDocumentiFascicolo_rowNum -1}'); " >
					<img class="padd_sx_logo" src="<c:url value="/public/img/ico_view_det.png"/>" alt="vaiVis" title="Visualizza Metadati ${listDocumentiFascicolo.descDocumento.textDescrizione}" >
				</a>
			</demdec:displaytag-column>
...
<div class="row center" >
	<demdec:submit keyValue="button.back" controllerCode="backToFadInitialPageFromRed?idFad=${idFadAperto}" />
</div>	
