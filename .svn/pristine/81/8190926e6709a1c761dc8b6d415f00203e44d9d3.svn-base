...
public class MailManagerImpl extends AbstractDemManagerImpl implements MailManager {
	...
	/**
	 * A monte di tutto viene eseguito un controllo se la funzione che sta chiamando il metodo è abilitata all'invio della PEC.
	 * Il metodo viene chiamato dalla relativa funzione che necessita dell'invio di PEC. Viene impostato l'oggetto della email con il valore contenuto nella variabile
	 * textOggetto e il testo con il valore contenuto nella variabile textMail, indipendentemente dai valori standard associati alla funzione. Quindi viene eseguito   
	 * l'invio del messaggio. Attualmente è impostata in modo statico la funzione TESTPEC. Dopo essere stato inviato alcuni dati del messaggio vengono salvati sulla 
	 * tabella D_PEC_MESSAGGI_INVIATI.
	 * 
	 *  Nella versione definitiva il metodo probabilmente sarà void (da definire più avanti qunado ci saranno le funzioni sviluppate)
	 *  
	 *  NOTE: i parametri di connessione sono impostati nel bean pec e vengono presi dalla tabella D_TAB_SERV_DEM
	...
	public MimeMessage invioPecDaFunzione(String codiFunzione, Utente utenteInSessione, String address, String textOggetto, String textMail) throws Throwable{
		...
		MailTestiTO mailTesti = getDozerDriver().map(mailTestiDao.findMailTestiByCodiFunzione(codiFunzione), MailTestiTO.class, "Mail");
		mailTesti.setTextOggettoEmail(textOggetto);
		mailTesti.setTextEmail(textMail);
