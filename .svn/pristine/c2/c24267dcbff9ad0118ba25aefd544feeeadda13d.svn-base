...
@RequestMapping(value = ConstantsRequestMapping.BOLLINA_DOCUMENTI)
public class BollinaDocumentiController extends AbstractDemFormController{
	...
	public String startController(Context context) throws Throwable {
		...
		context.setCurrentNode("DemDec.menu.Bollinatura");
		...
		listAttoDecretoToSign.addAll(this.documentiManager.listADToSignOrVerifyByLoggedUser(getUtenteInSessione(), Constants.BOLLINATURA));
	 ...
	 * @throws Throwable
	 */
	public String bollina(Context context) throws Throwable {
		...
		if(form.isValid() && havePin(context)){
			...
			for (int i = 0; i < form.getSelezioni().length; i++) {
				...
				context.valorizeAudit(selectedDocument, NomeFunzioneConstants.BOLLINA_DOCUMENTI );
				try {
					IterFirmaDecretoTO ifd = documentiManager.sigla(selectedDocument, getUtenteInSessione(), context.getRequest(), context.getResponse(), form.getSelezioni().length, CroTransTO.CRO_TRANS_BOLLINA_DOCUMENTI);
					...
					if(ifd != null){
						...
						if(ifd.getAnagFirmatari().getFlagAbilitaMail01() != Constants.DESTINATARIO_MAIL_DISABILITATO){
							if((ifd.getAnagFirmatari().getFlagPec01() == HAVE_PEC)){
								message = mailManager.invioPecDaFunzione(NomeFunzioneConstants.BOLLINA_DOCUMENTI, getUtenteInSessione(), email);
							}else{
								message = mailManager.invioPeoDaFunzione(NomeFunzioneConstants.BOLLINA_DOCUMENTI, getUtenteInSessione(), email);
								
							}
						}
						if(message != null){
							PecMessaggiInviatiTO messageToSave = mailManager.createMessageToSave(message,NomeFunzioneConstants.BOLLINA_DOCUMENTI);
							context.valorizeAuditCreated(messageToSave, NomeFunzioneConstants.BOLLINA_DOCUMENTI);
							context.valorizeAuditUpdated(messageToSave, NomeFunzioneConstants.BOLLINA_DOCUMENTI);
							mailManager.salvaMessaggioInviato(messageToSave, getUtenteInSessione());
							context.addInfo("info.doc.bollinato.and.msg.sent", selectedDocument.getDecreto(), mailManager.recipientsToString( message.getAllRecipients()));
						}else{
							context.addInfo("info.doc.bollinato", selectedDocument.getDecreto());
	 ...
	 * @return
	 */
	private Boolean havePin(Context context) throws Throwable{
		if(getUtenteInSessione().getUtente().getCodiPinFirma() == null || getUtenteInSessione().getUtente().getCodiPinFirma().equals("")){
			context.addError("error.no.pinencrypted",  ConstantsRequestMapping.BOLLINA_DOCUMENTI);
			return false;
		}
		return true;
	}
	...
	public String findMySignedDocuments(Context context) throws Throwable{
		...
		List<DocumentiViewTO> listMySignedDocuments = documentiManager.listSignedADByLoggedUser(getUtenteInSessione(), Constants.BOLLINATURA);
...
//		List<DocumentiViewTO> listADSigned = documentiManager.listSignedADByLoggedUser(getUtenteInSessione(), Constants.BOLLINATURA);
